<template>
  ...
  <view class="city-select">
    <scroll-view :scroll-top="scrollTop" scroll-y="true" class="city-select-main" id="city-select-main">
      ...
      <view class="hot-content" v-if="!serachCity">
        <!-- 当前定位城市 -->
        <view class="hot-title-content" v-if="activeCity && !serachCity" @click="cityTrigger(activeCity)">
          <app-icon type="positioning" :size="28" color="#9D9D9D"></app-icon>
          <view class="hot-title">当前定位城市 : {{ activeCity.name }}</view>
        </view>
        <view class="line" v-if="hotCity.length > 0 && !serachCity"></view>
        <!-- 热门城市 -->
        <view  class="hot-title-content" v-if="hotCity.length > 0 && !serachCity">
  ...
  computed: {
    ...
    searchDatas () {
      ...
      for (let i = 0; i < this.cityData.length; i++) {
        if (this.cityData[i][this.formatName].indexOf(this.serachCity) !== -1 || this.cityData[i].tags.indexOf(this.serachCity) !== -1 ) {
          searchData.push({
